...
public class TokenController {
	...
	public List<String> getTokens() {
		...
		final Collection<OAuth2AccessToken> tokens = this.tokenStore.findTokensByClientId("sampleClientId");
		if (tokens != null)
	...
	public List<String> getTokens(@PathVariable String clientId) {
		final List<String> tokenValues = new ArrayList<>();
		final Collection<OAuth2AccessToken> tokens = this.tokenStore.findTokensByClientId(clientId);
		if (tokens != null)
			for (final OAuth2AccessToken token : tokens)
				tokenValues.add(token.getValue());
		return tokenValues;
